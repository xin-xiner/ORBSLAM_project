...
Frame::Frame(const std::vector<cv::Mat> &imGrays, const double &timeStamp, ORBextractor* extractor, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth,std::vector<cv::Mat>& Tcm)
{
	// Frame ID
	mnId = nNextId++;
	...
	mvTcm.resize(Tcm.size());
	for (int i = 0; i < Tcm.size(); i++)
	{
		Tcm[i].copyTo(mvTcm[i]);
	}
	// ORB extraction
